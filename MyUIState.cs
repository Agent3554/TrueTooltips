    ...
    class MyUIState : UIState
    {
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            for (int i = 0, num = 20; i < 10; i++)
            {
                ...
                {
                    TextPulse(RarityColor(LocalPlayer.inventory[i]), out Color color, out _);

                    ChatManager.DrawColorCodedStringWithShadow(spriteBatch, fontMouseText, LocalPlayer.inventory[i].HoverName, new Vector2(x, y), color, 0, Vector2.Zero, Vector2.One);
            ...
            for (int i = 400; i >= 0; i--)
            {
                ...
                if (item[i].active && new Rectangle((int)item[i].position.X + item[i].width / 2 - itemTexDim.Width / 2, (int)item[i].position.Y + item[i].height - itemTexDim.Height, itemTexDim.Width, itemTexDim.Height).Contains((int)screenPosition.X + mouseX, (int)screenPosition.Y + mouseY) && !mouseText)
                {
                    ...
                    TextPulse(RarityColor(item[i]), out Color color, out _);

                    ChatManager.DrawColorCodedStringWithShadow(spriteBatch, fontMouseText, item[i].HoverName, new Vector2(x + itemNameLength.X + 4 > screenWidth ? screenWidth - itemNameLength.X - 4 : x, y + itemNameLength.Y + 4 > screenHeight ? screenHeight - itemNameLength.Y - 4 : y), color, 0, Vector2.Zero, Vector2.One);
